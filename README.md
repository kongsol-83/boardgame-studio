...
보드게임 제작은 한 가지 작업이 아닙니다. 레퍼런스 조사, 시스템 설계, 룰 작성, 컴포넌트 사양,
밸런스 검토, 아트 디렉션에 전부 다른 종류의 판단이 필요합니다. 이 저장소는 Cursor 세션에
...
돌아갑니다. 룰셋에는 버전을 붙입니다. 어느 변경이 효과가 있었는지 나중에 짚어야 하니까요.
...
20x20mm 토큰처럼 작은 건 이미지 모델의 최소 해상도를 넘기려고 키운 뒤 인쇄할 때 줄이고,
...
진짜 비용인 데다 나쁜 봇은 잘못된 결론을 냅니다. 게다가 LLM은 학습 데이터에 없는 새 게임을
...
다루지 못해서, 이 저장소에서 작업하면 원인을 찾기 어려운 오류를 만납니다.
...
이 설정과 무관하게 한국어를 유지합니다. 아는 값은 `ko`, `en`, `ja`, `zh-CN`, `de`, `fr`,
`es` 이고, 다른 값을 넣어도 막지 않고 경고만 합니다.
