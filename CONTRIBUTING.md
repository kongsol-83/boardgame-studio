...
생성되는 설계 문서는 다릅니다. 언어를 `studio.config.json` 의 `language` 로 직접 정합니다.
쓰는 분의 나라가 다를 수 있어서 열어뒀습니다. 스킬과 에이전트는 이 값을 읽어 그 언어로
문서를 씁니다.

영어 문서를 늘리는 기여는 환영하지만 **한국어 원본을 영어로 갈아치우는 것은 받지 않습니다.**
나란히 두는 쪽으로 부탁드립니다. Issue와 PR은 한국어와 영어 모두 괜찮습니다.
...
정사각 타일처럼 실제로 쓰이는 규격이면 무엇이든 좋습니다. 진입장벽이 가장 낮은데 모두에게
바로 쓸모가 있습니다. 출처(제작사 스펙 시트 등)를 같이 적어주세요.
...
그렇게 되면 폴더를 없애고 한 층으로 되돌리면 됩니다. 이름은 경로와 무관하므로 다른 파일은
하나도 고칠 필요가 없습니다.
...
이걸로 만든 게임 이야기, 잘 안 된 대목, 워크플로우가 어디서 막혔는지. Issue나 Discussion으로
남겨주시면 가장 도움이 됩니다.
...
- **Windows 경로를 가정하지 않습니다.** 개발은 Windows에서 하지만 `node:path` 를 쓰고
  하드코딩된 구분자를 넣지 않습니다.
- **API 키가 필요한 코드는 CI에서 못 돕니다.** 순수 로직을 분리해서 테스트할 수 있게
  해주세요.
...
납니다. 두 셸을 실제로 비교한 표는
[README의 PowerShell 절](README.md#windows라면-powershell-7을-먼저-설치하세요)에 있습니다.
...
CI도 같은 명령을 ubuntu와 windows 두 러너에서 돌리고, 여기에 커밋 금지 항목 검사를 더합니다.
