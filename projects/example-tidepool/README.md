...
**실제 게임이 아니라 파이프라인을 보여주는 예제입니다.** 재미있게 만들려고 설계하지
않았습니다. 도구가 무엇을 읽고 무엇을 내는지 눈으로 보려고 만들었습니다.

> 이 문서는 **무엇이 심어져 있는지 적어둔 목록**입니다. 명령을 따라 치면서 하나씩 직접
> 걸려보려면 [TUTORIAL.md](TUTORIAL.md) 로 가세요. **여기에는 답이 먼저 적혀 있으니**
> 그쪽을 먼저 하는 편이 낫습니다.
...
CI가 매번 이 명령들을 실행합니다. 도구가 깨지면 여기서 걸립니다.
...
도구가 무엇을 잡아내는지 보여주려고 일부러 심어둔 것들입니다.

하나 미리 말씀드리면, **`spec.json` 의 컴포넌트 중 일부는 규격 도구를 보여주기 위한
소품이라 룰에서 쓰이지 않습니다.** 지형 타일, 메인 보드, 물때 표시 띠가 그렇습니다. 규격
해석기의 서로 다른 경로를 한 프로젝트에서 다 보려고 넣었습니다. 실제 프로젝트라면
결함입니다. `bgs-rules-critic` 도 "구성물의 절반이 용도 불명"이라고 정확히 짚어냅니다.

- **`TP05 무리한 잠수`** — 3코스트에 파워 12입니다. 파워 단독 분포에서는 이상치가
  아니지만 코스트 대비로 보면 튑니다. `balance.mjs` 의 회귀 잔차가 이 카드를 집어냅니다
...
`ruleset.md` 에는 답이 빠진 자리가 여럿 있습니다. 전부 룰북을 쓸 때 실제로 빠뜨리기 쉬운
것들입니다.
...
- **시작 플레이어를 정하는 규칙이 없습니다** — "1번 플레이어부터"라고만 하고 누가 1번인지는
  정하지 않습니다. 매 게임 첫 순간에 멈춥니다
- **덱이 모자랄 때 보충 순서가 없습니다** — 완전히 빈 경우만 다룹니다. 4인이면 5라운드쯤에
  남은 한두 장을 누가 갖느냐로 점수가 갈리는데 답이 없습니다
- **전원 채집력이 0인 라운드** — 한 명만 채집력 0짜리 카드를 내고 나머지가 쉬었을 때,
  그 사람이 최고인지 전원 동점인지 갈립니다
- **낸 카드가 공개인지** — 누적 채집력을 비교하려면 남의 카드가 보여야 하는데 언급이 없습니다
- **용어가 섞여 있습니다** — 물때와 라운드, 채집물과 채집력과 자원

이 구멍들은 **일부러 안 막아뒀습니다.** 막아버리면 크리틱이 할 일이 없어집니다. 자기
게임이라면 여기서 나온 항목을 룰북에 반영하고 버전을 올리면 됩니다.
...
30판 동안 한 번도 언급되지 않았습니다. 드문 상황인 데다, 플레이어는 막히면 그냥 그럴듯하게
정하고 넘어가기 때문입니다.
